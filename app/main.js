...
const dom = {
  ...
  infoMenuBtn: document.getElementById("infoMenuBtn"),
  ...
  turnstileWrap: document.getElementById("turnstileWrap"),
  turnstileStatus: document.getElementById("turnstileStatus"),
  turnstileContainer: document.getElementById("turnstileContainer"),
  signInUnlockHint: document.getElementById("signInUnlockHint"),
  signInBtnWrap: document.getElementById("signInBtnWrap"),
  gsiButtonContainer: document.getElementById("gsiButtonContainer"),
...
let lastGsiRenderKey = "";

const I18N = {
  en: {
    heroBadge: "4D / 5D Construction Viewer",
    heroSubtitle:
      "Explore schedule, costs and 3D progress in one interactive workspace.",
    statusLabel: "Session",
    info: "Info",
    infoTitle: "Project links",
    githubRepo: "GitHub repository",
    youtubeDemo: "YouTube video",
    account: "Account",
    ...
    turnstilePrompt: "Verify that you are human to unlock sign in.",
    turnstileReady: "Verification passed. Sign in is unlocked.",
    turnstileExpired: "Verification expired. Complete Turnstile again.",
    turnstileFailed: "Verification failed. Try Turnstile again.",
    signInLocked: "Sign in becomes available after Turnstile verification.",
  ...
  de: {
    heroBadge: "4D / 5D Bau-Viewer",
    heroSubtitle:
      "Plane Termine, Kosten und 3D-Fortschritt gemeinsam in einem interaktiven Arbeitsbereich.",
    statusLabel: "Sitzung",
    info: "Info",
    infoTitle: "Projektlinks",
    githubRepo: "GitHub-Repository",
    youtubeDemo: "YouTube-Video",
    account: "Konto",
  ...
  ru: {
    heroBadge: "4D / 5D просмотр строительства",
    heroSubtitle:
      "Изучайте график, затраты и 3D-прогресс в едином интерактивном пространстве.",
    statusLabel: "Сессия",
    info: "Инфо",
    infoTitle: "Ссылки проекта",
    githubRepo: "Репозиторий GitHub",
    youtubeDemo: "Видео YouTube",
    account: "Аккаунт",
  ...
  uk: {
    heroBadge: "4D / 5D перегляд будівництва",
    heroSubtitle:
      "Переглядайте графік, витрати та 3D-прогрес в єдиному інтерактивному просторі.",
    statusLabel: "Сесія",
    info: "Інфо",
    infoTitle: "Посилання проєкту",
    githubRepo: "Репозиторій GitHub",
    youtubeDemo: "Відео YouTube",
    account: "Акаунт",
...
function applyTranslations() {
  if (dom.heroBadge) dom.heroBadge.textContent = t("heroBadge");
  if (dom.heroSubtitle) dom.heroSubtitle.textContent = t("heroSubtitle");
  if (dom.statusLabel) dom.statusLabel.textContent = t("statusLabel");
  if (dom.infoMenuBtn) dom.infoMenuBtn.textContent = t("info");
  ...
  if (state.auth.googleReady && state.auth.gsiClientId && !state.auth.user) {
    lastGsiRenderKey = "";
    renderGsiSignInButton(state.auth.gsiClientId);
  }
...
function apiUrl(pathname) {
  const isLocalBackend =
    ...
    window.location.port === "7002";
  if (isLocalBackend) return pathname;
...
async function initAuth() {
  ...
  await setupGoogleAuthMenu(config?.googleClientId);
  attachAuthHandlers();
...
function mergeGoogleClientIdFromConfig(apiConfig) {
  const data = { ...apiConfig };
  const fromApi = String(data.googleClientId || "").trim();
  const fromFile = String(CONFIG.googleClientId || "").trim();
  if (!fromApi && fromFile) data.googleClientId = fromFile;
  const fromTurnstileApi = String(data.turnstileSiteKey || "").trim();
  const fromTurnstileFile = String(CONFIG.turnstileSiteKey || "").trim();
  if (!fromTurnstileApi && fromTurnstileFile) {
    data.turnstileSiteKey = fromTurnstileFile;
  }
  return data;
}
...
async function fetchConfig() {
  try {
    ...
    if (!response.ok) {
      ...
      updateServerStatusBanner();
      return mergeGoogleClientIdFromConfig({});
    }
    state.auth.configError = "";
    state.auth.serverAvailable = true;
    updateServerStatusBanner();
    return mergeGoogleClientIdFromConfig(await response.json());
  } catch (error) {
...
function setupGoogleAuthMenu(clientId) {
  ...
  if (!cid) {
    ...
    state.auth.gsiClientId = null;
    return;
  }
  if (state.auth.gsiClientId && state.auth.gsiClientId !== cid) {
    state.auth.googleReady = false;
    state.auth.gsiClientId = null;
  }
  ensureGoogleReady(cid);
}
...
function isTurnstileReadyForSignIn() {
  return !state.auth.turnstileRequired || state.auth.turnstileVerified;
}

function updateTurnstileStatus() {
  if (!dom.turnstileStatus) return;
  if (!state.auth.turnstileRequired) {
    dom.turnstileStatus.textContent = "";
    return;
  }
  dom.turnstileStatus.textContent = state.auth.turnstileVerified
    ? t("turnstileReady")
    : t("turnstilePrompt");
}

function clearTurnstileState(options = {}) {
  const keepRequirement = options.keepRequirement === true;
  state.auth.turnstileVerified = false;
  state.auth.turnstileToken = "";
  if (!keepRequirement) {
    state.auth.turnstileRequired = false;
    state.auth.turnstileSiteKey = null;
  }
  updateTurnstileStatus();
}

function resetTurnstileWidget() {
  clearTurnstileState({ keepRequirement: true });
  if (
    state.auth.turnstileRequired &&
    state.auth.turnstileWidgetId != null &&
    window.turnstile?.reset
  ) {
    window.turnstile.reset(state.auth.turnstileWidgetId);
  }
}

function loadTurnstileScript() {
  if (window.turnstile?.render) return Promise.resolve();
  if (state.auth.turnstileInitPromise) return state.auth.turnstileInitPromise;
  state.auth.turnstileInitPromise = new Promise((resolve, reject) => {
  ...
  return state.auth.turnstileInitPromise;
}

function renderTurnstileWidget(siteKey) {
  const key = String(siteKey || "").trim();
  if (!key || !dom.turnstileContainer || !window.turnstile?.render) return;
  if (state.auth.turnstileWidgetId != null) return;
  dom.turnstileContainer.innerHTML = "";
  state.auth.turnstileWidgetId = window.turnstile.render(
    dom.turnstileContainer,
    {
      sitekey: key,
      theme: "light",
      callback: (token) => {
        state.auth.turnstileVerified = true;
        state.auth.turnstileToken = String(token || "").trim();
        updateAuthUi();
      },
      "expired-callback": () => {
        clearTurnstileState({ keepRequirement: true });
        if (dom.turnstileStatus) dom.turnstileStatus.textContent = t("turnstileExpired");
        updateAuthUi();
      },
      "timeout-callback": () => {
        clearTurnstileState({ keepRequirement: true });
      ...
      "error-callback": () => {
        clearTurnstileState({ keepRequirement: true });
        if (dom.turnstileStatus) dom.turnstileStatus.textContent = t("turnstileFailed");
        updateAuthUi();
      },
    }
  );
...
async function setupTurnstile(siteKey) {
  ...
  state.auth.turnstileSiteKey = key;
  state.auth.turnstileRequired = true;
  try {
    await loadTurnstileScript();
    renderTurnstileWidget(key);
  } catch (error) {
    if (dom.turnstileStatus) {
      dom.turnstileStatus.textContent = t("turnstileFailed");
    }
  }
  updateAuthUi();
}
...
function gsiButtonLocale(lang) {
  const map = { en: "en", de: "de", ru: "ru", uk: "uk" };
  return map[lang] || "en";
}

function renderGsiSignInButton(clientId) {
  const cid = String(clientId || "").trim();
  if (!dom.gsiButtonContainer || !cid || !window.google?.accounts?.id) return;
  const key = `${cid}|${state.locale}`;
  if (key === lastGsiRenderKey && dom.gsiButtonContainer.childNodes.length) return;
  lastGsiRenderKey = key;
  dom.gsiButtonContainer.innerHTML = "";
  window.google.accounts.id.renderButton(dom.gsiButtonContainer, {
    type: "standard",
    theme: "outline",
    size: "large",
    text: "signin_with",
    shape: "rectangular",
    width: 220,
    locale: gsiButtonLocale(state.locale),
...
async function ensureGoogleReady(clientId) {
  ...
  try {
    ...
    const already = state.auth.gsiClientId === cid && state.auth.googleReady;
    if (!already) {
      ...
      state.auth.gsiClientId = cid;
      state.auth.googleReady = true;
      lastGsiRenderKey = "";
    }
    renderGsiSignInButton(cid);
    return true;
  } catch (error) {
    state.auth.googleReady = false;
    state.auth.gsiClientId = null;
...
function attachAuthHandlers() {
  if (dom.accountMenuDetails) {
    dom.accountMenuDetails.addEventListener("toggle", () => {
      if (dom.accountMenuDetails.open) {
...
function parseGoogleIdTokenPayload(credential) {
  try {
...
function isGoogleIdTokenExpired(credential) {
...
async function handleGoogleCredential(response) {
  ...
  if (!isTurnstileReadyForSignIn()) {
    alert(t("turnstilePrompt"));
    return;
  }
...
async function exchangeGoogleCredential(credential, options = {}) {
  ...
  try {
    const response = await fetch(apiUrl("/api/auth/google"), {
      ...
      body: JSON.stringify({
        credential,
        turnstileToken: state.auth.turnstileToken,
      }),
    });
    const payload = await response.json();
    if (!response.ok) {
      if (silent) {
        const silentErr = String(payload.error || "");
        if (silentErr) {
          console.warn("Google auth (session restore failed):", silentErr);
        }
        clearAuthSession();
        return;
      }
...
function setAuthSession(token, user) {
  ...
  if (token) {
    authStorage.setItem("id_token", token);
  }
  if (user) {
    authStorage.setItem("user_profile", JSON.stringify(user));
    resetTurnstileWidget();
  }
  updateAuthUi();
  const pendingSubscription = authStorage.getItem(
    "paypal_pending_subscription"
  );
  if (pendingSubscription && token) {
    void capturePaypalSubscription(pendingSubscription, { silent: true });
  }
}
...
function clearAuthSession() {
  ...
  authStorage.removeItem("id_token");
  authStorage.removeItem("user_profile");
  resetTurnstileWidget();
  updateAuthUi();
}

async function restoreAuthSession() {
  const token = authStorage.getItem("id_token") || "";
  if (!token) return;
  ...
  state.auth.token = token;
  try {
    const rawUser = authStorage.getItem("user_profile");
    state.auth.user = rawUser ? JSON.parse(rawUser) : null;
  } catch (error) {
    state.auth.user = null;
  }
  updateAuthUi();
...
async function refreshMe() {
  ...
  try {
    ...
    authStorage.setItem("user_profile", JSON.stringify(state.auth.user));
    updateAuthUi();
  } catch (error) {
    // ignore
  }
...
function updateAuthUi() {
  ...
  if (dom.signInBtnWrap) {
    const showGsi =
      !user &&
      state.auth.googleReady &&
      state.auth.gsiClientId &&
      isTurnstileReadyForSignIn();
    dom.signInBtnWrap.classList.toggle("hidden", !showGsi);
  }
  if (dom.turnstileWrap) {
    dom.turnstileWrap.classList.toggle(
      "hidden",
      Boolean(user) || !state.auth.turnstileRequired
    );
  }
  if (dom.signInUnlockHint) {
    const showHint =
      !user &&
      state.auth.turnstileRequired &&
      !state.auth.turnstileVerified;
    dom.signInUnlockHint.classList.toggle("hidden", !showHint);
    dom.signInUnlockHint.textContent = t("signInLocked");
  }
  if (dom.logoutBtn) {
...
function scheduleServerCheck() {
  setInterval(async () => {
    ...
    if (latest?.googleClientId) {
      ...
      await setupTurnstile(latest?.turnstileSiteKey);
      await setupGoogleAuthMenu(latest?.googleClientId);
      updateAuthUi();
    }
  }, 60000);
}
...
async function handlePaypalReturn() {
  ...
  authStorage.setItem("paypal_pending_subscription", subscriptionId);
  if (!state.auth.token) {
    return;
  }
...
async function capturePaypalSubscription(subscriptionId, options = {}) {
  const silent = options.silent === true;
  try {
    ...
    const payload = await response.json().catch(() => ({}));
    if (!response.ok) {
      authStorage.removeItem("paypal_pending_subscription");
      if (silent) {
        console.warn(
          "PayPal subscription sync failed (pending id cleared):",
    ...
    authStorage.removeItem("paypal_pending_subscription");
    if (silent) {
      console.warn("PayPal subscription sync error (pending id cleared):", error);
    } else {
